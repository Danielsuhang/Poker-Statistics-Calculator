...
public class Game {
	...
	public void setGoodPokerHand(PokerHand[] goodRange) {
		gen.setGoodHandRange(goodRange);
		goodrange = goodRange;
	}
	public PokerHand generateGoodPokerHands() {
		if (goodrange == null) {
			System.err.println("ERROR: Hand range has not been defined yet");
			return null;
		}
		PokerHand test = generatePair();
		while (goodrange != null) {
			for (int i = 0; i < goodrange.length; i++) { //Iterates through all the goodranges and sees if any match
				if (test.getCard1().contains(goodrange[i].getCard1())) {
					if (test.getCard2().contains(goodrange[i].getCard2())) {
						if ((test.getSuited() && checkSuited(test)) || (!test.getSuited() && !checkSuited(test))) {
							return new PokerHand(test.getCard1(), test.getCard2());   //Suits are in the range 
						}
						
					}
				}
				else if (test.getCard1().contains(goodrange[i].getCard2())) {
					if (test.getCard2().contains(goodrange[i].getCard1())) {
						if ((test.getSuited() && checkSuited(test)) || (!test.getSuited() && !checkSuited(test))) {
							return new PokerHand(test.getCard2(), test.getCard1());   //Suits are in the range 
						}
					}
				}
				else {
					//Do nothing, since the hand doesn't match
				}
			}
			usedHands.remove(test.getCard1());   //Because we don't want to count this generated Hand
			usedHands.remove(test.getCard2());
			test = generatePair();   //Tries to generate another hand that is in the range
		}
		return null;
	}
	...
	public boolean checkSuited(PokerHand hand) {
		if (hand.getCard1().toUpperCase().contains("DIAMOND")) {
			return hand.getCard2().toUpperCase().contains("DIAMOND");
		}
		else if (hand.getCard1().toUpperCase().contains("SPADE")) {
			return hand.getCard2().toUpperCase().contains("SPADE");
		}
		else if (hand.getCard1().toUpperCase().contains("HEART")) {
			return hand.getCard2().toUpperCase().contains("HEART");
		}
		else if (hand.getCard1().toUpperCase().contains("CLUB")) {
			return hand.getCard2().toUpperCase().contains("CLUB");
		}
		else {
			System.err.println("ERROR: No suit specified");
			return false;
		}
	}
